...
<script>
  import {conditionData} from '../../../common/js/jobData';
  export default {
    data(){
      return{
        option1:{
      ...
      selectedOption(option){
        ...
        if(option.check){
          if(option.type ===0){
            ...
            this.options.option3.forEach((item)=>{
                if(item.optionName != option.optionName){
                  item.check = false;
                }
            })
          }
